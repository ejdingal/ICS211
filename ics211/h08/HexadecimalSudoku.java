 ...
 * ICS 211 -- Modified by Emeri Joe Dingal, used and modified some code from GitHub, @ https://goo.gl/dwf878. Worked with
 * Kason Shiroma, Kade Shiroma, Angeli Amascual
 */
public class HexadecimalSudoku {
  ...
  private static boolean solveSudoku(int[][] sudoku, int row, int col) {
    ArrayList<Integer> validInts = legalValues(sudoku, row, col); // list of valid ints
    for (Integer i : validInts) { // cycle through ints
      sudoku[row][col] = i; // place int into cell
      if (solveSudoku(sudoku)) {
        return checkSudoku(sudoku, false); // check if it passed sudoku rules
      }
      else { // did not pass
        sudoku[row][col] = -1; // revert
      }
    }
  ...
  public static ArrayList<Integer> legalValues(int[][] sudoku, int row, int column) {
    ...
    for (int i = 0; i < sudoku.length; i++) { // columns
    ...
    for (int j = 0; j < sudoku.length; j++) { // rows
